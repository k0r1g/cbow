...
# --- Point to the ACTUAL Word2Vec checkpoint generated by 01_train_w2v.py ---
# CBOW_CKPT = "checkpoints/hn_cbow_finetuned.pth" # Incorrect .pth path
# CBOW_VOCAB_PATH = "finetuned_vocab.pkl"          # Not needed for .word2vec
CBOW_CKPT = "checkpoints/2025_04_18__12_41_13.hn-w2v-ft.epoch2.word2vec" # Correct path to epoch 2 checkpoint
from pathlib import Path
assert Path(CBOW_CKPT).is_file(), f"❌ File not found: {CBOW_CKPT}"
PAD_IDX = 0 # Keep PAD_IDX consistent (usually 0)
EPOCHS = 3
BATCH_SIZE = 512
...
# ---------------------------------------------------------------------------
# 2️⃣  Down‑sample score=1 rows so |score==1| == |score>1|
# ---------------------------------------------------------------------------
# ---- SKIPPING DOWNSAMPLING STEP ---
# ones = items[items.score == 1]  # Select rows with score == 1
# greater_than_one = items[items.score > 1]  # Select rows with score > 1
# ones_sampled = ones.sample(n=len(greater_than_one), random_state=42) # Sample the score == 1 group
# balanced = pd.concat([greater_than_one, ones_sampled]).sample(frac=1, random_state=42) # Combine and shuffle
# print("Balanced set (score=1 vs score>1):", Counter(balanced.score == 1)) # Update print statement
print("Skipping downsampling. Using the full, unbalanced dataset.")
# --- The variable 'items' now holds the full dataset to be used directly ---

# ---------------------------------------------------------------------------
# 3️⃣  Build month‑level mean scores (temporal feature)
# ---------------------------------------------------------------------------
# Use the full 'items' DataFrame instead of 'balanced'
month_mean = items.groupby("ym").score.mean().to_dict()
items["month_avg"] = items["ym"].map(month_mean)
...
# Use the full 'items' DataFrame instead of 'balanced'
items["ids"] = items.title.apply(tokens_to_ids)

# Display a sample of original vs tokenized
print("\nSample tokenization:")
# Use the full 'items' DataFrame instead of 'balanced'
sample_idx = items.sample(1).index[0]
print("Original Title:", items.loc[sample_idx, 'title'])
print("Token IDs:", items.loc[sample_idx, 'ids'])
...
# Use the full 'items' DataFrame instead of 'balanced'
train_df = items.sample(frac=0.8, random_state=SEED)
temp_df = items.drop(train_df.index)
...
# Call CBOW without vocab_path - it will detect .word2vec and load correctly
cbow = model.CBOW(CBOW_CKPT, trainable=False).eval()

# Get embedding dim directly from the loaded model instance
emb_dim = cbow.embedding_dim
print(f"CBOW model loaded. Embedding dimension: {emb_dim}")
